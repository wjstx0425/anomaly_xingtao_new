...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--template-dir", type=Path, required=True, help="Directory created by build_geometry_templates.")
    ...
    parser.add_argument("--calibrate-thresholds", action="store_true", help="Write slot thresholds from this normal set.")
    parser.add_argument(
        "--threshold-mode",
        choices=("region", "slot"),
        default="region",
    ...
    parser.add_argument("--tolerance-px", type=int, default=3, help="Pixel tolerance before less/more component scoring.")
...
def _threshold_rows(
    ...
    thresholds: dict[str, float] | dict[tuple[str, str, str], float],
    margin_ratio: float,
    threshold_mode: str,
) -> list[dict[str, str | int | float]]:
    """Return per-slot threshold rows."""
    rows = []
    if threshold_mode == "slot":
        grouped: dict[str, list[float]] = defaultdict(list)
        for score in scores:
            grouped[score.slot].append(float(score.geometry_score))
        for slot in sorted(thresholds):
            values = grouped.get(str(slot), [])
            ...
            rows.append(
                {
                    ...
                    "geometry_threshold": thresholds[slot],
...
def evaluate_geometry(args: argparse.Namespace) -> Path:
    ...
    thresholds: dict[tuple[str, str, str], float] | None = None
    if args.thresholds:
    ...
    if args.calibrate_thresholds:
        if args.threshold_mode == "slot":
            ...
            thresholds = {(slot, "*", "*"): threshold for slot, threshold in slot_thresholds.items()}
            threshold_rows = _threshold_rows(scores, slot_thresholds, args.threshold_margin, args.threshold_mode)
        else:
            thresholds = geometry.calibrate_region_thresholds(scores, margin_ratio=args.threshold_margin)
            threshold_rows = _threshold_rows(scores, thresholds, args.threshold_margin, args.threshold_mode)
        geometry.write_csv(
