...
def _artifact(path_value: str | None, *, hash_source: bool = False) -> tuple[bool | None, str | None]:
    """Return artifact existence and an optional source digest."""
    if path_value is None:
        return None, None
    path = Path(path_value)
    exists = path.is_file()
    return exists, sha256_file(path) if exists and hash_source else None
