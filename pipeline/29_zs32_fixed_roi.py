...
"""Pipeline stage 29: select eight fixed ZS32 ROIs and crop the YOLO dataset."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    select_parser = subparsers.add_parser("select", help="Open eight OpenCV windows and save the fixed ROI config.")
    select_parser.add_argument("--repo-root", type=Path, default=REPO_ROOT)
    select_parser.add_argument("--input-root", type=Path, default=dataset_root / "zs32_eight_view_yolo")
    select_parser.add_argument("--config", type=Path, default=dataset_root / "zs32_eight_view_roi_config.json")
    select_parser.add_argument("--preview-dir", type=Path, default=dataset_root / "zs32_eight_view_roi_previews")
    ...
    convert_parser.add_argument("--input-root", type=Path, default=dataset_root / "zs32_eight_view_yolo")
    convert_parser.add_argument("--config", type=Path, default=dataset_root / "zs32_eight_view_roi_config.json")
    convert_parser.add_argument("--output-root", type=Path, default=dataset_root / "zs32_eight_view_roi_yolo")
