...
def discover(root: Path):
    ...
    for p in root.rglob("*"):
        ...
        rows.append({
            "view": f"left_{position}", "slot": f"slot{int(sm.group(1)):02d}",
            "label": label, "split": "train" if label == "normal" else "test",
            "path": str(p.resolve()),
        })
...
def choose_templates(paths, width, count, max_samples):
    ...
    thumb = np.stack([cv2.resize(im, (64, 32), interpolation=cv2.INTER_AREA).reshape(-1) for im in images]).astype(np.float32)
...
def main():
    ...
    model = {"method": "cv2.TM_CCOEFF_NORMED", "width": args.width, "shift": args.shift,
             "normal_quantile": args.normal_quantile, "groups": {}}
    predictions = []
    ...
    for view, slot in groups:
        ...
        for i, (source, image) in enumerate(selected, 1):
            ...
            cv2.imwrite(str(out), image); templates.append(image); template_files.append(str(out.relative_to(args.output)))
        ...
        key = f"{view}/{slot}"
        model["groups"][key] = {"threshold": threshold, "templates": template_files,
            "template_sources": [x[0] for x in selected], "train_count": len(train),
            "normal_test_count": len(validation), "defect_count": len(defects)}
        for r in validation + defects:
            score = best_score(r["path"], templates, args.width, args.shift)
            predictions.append({**r, "similarity": score, "threshold": threshold,
    ...
    with (args.output / "predictions.csv").open("w", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=predictions[0].keys()); w.writeheader(); w.writerows(predictions)
