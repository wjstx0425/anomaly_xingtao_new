...
- `pipeline/bmw_lab_prepare_trusted_ok_review.py --session-id <capture-session>` writes the ignored, no-overwrite review package. It creates `candidate_manifest.csv`, exactly one `PENDING` decision per physical part, Chinese-labelled 4x2 contact sheets, and `review_package.json` with `automatic_approvals: 0`; it can never approve a candidate automatically.
- Actual package `dataset/bmw_trusted_ok_review/bmw_right_20260810_21_train_normal_v1` was generated from session `20260810_210030_527506`: 50 candidate parts / 400 images / 50 contact sheets / 50 PENDING decisions. This is a manual review queue, not a trusted-OK approval or model acceptance result.
