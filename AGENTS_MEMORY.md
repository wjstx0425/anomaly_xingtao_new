...
- `TrustedOkMatcher` accepts the ignored `bmw_right_20260810_21_train_normal_approved_v2` release and explicitly refuses `approved_v1`. It binds `reference_index.json` and selected full/ROI images by SHA-256, applies the Template-compatible grayscale 512x512 fit, 3x3 blur, reflected padding, and a finite `TM_CCOEFF_NORMED` search with `max_shift=12`.
- `EightViewModelSuite` invokes trusted-reference matching only after all 25 model rows are complete and fused, and only once per unique NG/ERROR view. A bright-streak NG/ERROR uses the retained full `front_left` image; other actionable evidence uses the part ROI. PASS-only inspections perform no reference matching.
- Trusted matches and errors are immutable diagnostics on `EightViewInspection`; matcher failure never edits a branch result or changes `final_status`. Real v2 first-use matching over 50 references measured about 5.4-5.9 seconds for one uncached view and 0.29-0.44 seconds cached on this CPU environment, so first-use latency remains a Demo concern.
